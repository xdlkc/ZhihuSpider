...
class ZhiHuSpider(object):
    ...
    def login(self, username, password):
        ...
        # if 'r' in json.loads(result.text):
        #     # 要用验证码，post后登录
    ...
    def getQsAnswer(self, questionId):
        ...
        while record_num < total:
            ...
            if datas is not None:
                ...
                for data in datas:
                    answer_info.append("用户名:{}\n".format(data['author']['name']))
                    answer_info.append("链接:{}\n".format(data['url']))
                    answer_info.append("点赞人数：{}\n".format(data['voteup_count']))


                    dr = re.compile(r'<[^>]+>', re.S)
                    content = dr.sub('', data['content'])
                    # # answer = data['author']['name'] + ' ' + str(data['voteup_count']) + ' 人点赞' + '\n'
                    answer_info.append("原文：{}\n\n".format(content))
                    # answer_info.append('\n')
        ...
        print(title)
        self.write2File('/Users/zhangjunbo/Code/PYTHON/zhihu_spider/{}.txt'.format(questionId), answer_info)
