...
class NewtopicSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.zhihu.com/api/v3/topics/19776749/children?limit=10&offset=0']
    ...
    def parse(self, response):
        ...
        for data in data_json:
            ...
            if 'type' not in item:
                item['type'] = ''
            yield item
            yield scrapy.Request(url=self.url_model.format(topic_id, 0),
                                 meta={'offset': 0, 'parent_id': parent_id},
                                 callback=self.parse)
